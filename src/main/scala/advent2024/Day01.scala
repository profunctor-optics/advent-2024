...
object Day01 extends Day[(Int, Int)]:
  def parse(line: String): Option[(Int, Int)] = line.split("\\s+") match
    case Array(left, right) => Some((left.toInt, right.toInt))
    case _ => None

  private def totalDistance(locations: Iterator[(Int, Int)]) =
    val (left, right) = locations.toArray.unzip
    left.sortInPlace()
    right.sortInPlace()
    left.iterator.zip(right.iterator).map((l, r) => (r - l).abs.toLong).sum

  private def similarityScore(locations: Iterator[(Int, Int)]) =
    val (left, right) = locations.toArray.unzip
